  ...
  handler: async (params, context) => {
    ...
    try {
      switch (action) {
        case 'create':
          if (!name || !cron || !task) {
            return { success: false, error: 'name, cron, and task are required for create action.' };
          }
          const newId = uuidv4();
          ...
          return { success: true, data: { scheduleId: newId, message: `Schedule "${name}" created successfully.` } };
        ...
        case 'delete':
          if (!scheduleId) return { success: false, error: 'scheduleId is required for delete action.' };
          const delRes = await query('DELETE FROM schedules WHERE id = $1 AND agent_id = $2', [scheduleId, agentId]);
          if (delRes.rowCount === 0) return { success: false, error: 'Schedule not found or not owned by this agent.' };
          return { success: true, data: { message: 'Schedule deleted successfully.' } };

        case 'update':
          if (!scheduleId) return { success: false, error: 'scheduleId is required for update action.' };
          const currentRes = await query('SELECT * FROM schedules WHERE id = $1 AND agent_id = $2', [scheduleId, agentId]);
          if (currentRes.rows.length === 0) return { success: false, error: 'Schedule not found or not owned by this agent.' };
