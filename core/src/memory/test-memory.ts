...
export async function testMemory() {
  ...
  // gray-matter might not include title in frontmatter if not explicitly in metadata,
  // but it should definitely include the content and some metadata
  ...
  if (searchResults.length > 0 && searchResults[0]?.title.includes('test-archival-entry')) {
