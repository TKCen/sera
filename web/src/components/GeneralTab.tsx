...
import { useDefaultModel, useSetDefaultModel } from '@/hooks/useProviders';
...
export function GeneralTab({
  ...
  const { data: defaultModelData, isLoading: loaded } = useDefaultModel();
  const setDefaultModelMutation = useSetDefaultModel();

  useEffect(() => {
    if (defaultModelData?.defaultModel) {
      setDefaultModelLocal(defaultModelData.defaultModel);
    }
  }, [defaultModelData]);
      ...
      <div>
        ...
        <div className="flex gap-3 items-end">
          ...
          <Button
            ...
            disabled={setDefaultModelMutation.isPending || !defaultModel}
            onClick={() => {
              setDefaultModelMutation.mutate(defaultModel);
            }}
          >
            <Save size={13} /> Save
          </Button>
