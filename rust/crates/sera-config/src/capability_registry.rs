//! Capability policy registry — shared between gateway audit + runtime
//! pre-dispatch enforcement (sera-eo71).
//!
//! Loads `CapabilityPolicy` YAML files from a configured directory (default
//! `$XDG_CONFIG_HOME/sera/policies/` via [`crate::ConfigRoot`], override via
//! `SERA_CAPABILITY_POLICIES_DIR`), binds each loaded policy to the agents
...
/// [`crate::ConfigRoot`] — i.e. `$XDG_CONFIG_HOME/sera/policies` on
...
impl CapabilityRegistry {
    ...
    pub fn resolve_policies_dir() -> PathBuf {
        ...
        crate::ConfigRoot::from_env().policies_dir()
    }
