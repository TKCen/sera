...
export class SandboxManager {
  ...
  async spawn(
    ...
    // ── Bind Mounts ──────────────────────────────────────────────────────────
    const binds: string[] = await BindMountBuilder.buildBinds(
      manifest,
      request,
      caps,
      finalInstanceId,
    ...
    // ── Security & Resource Limits (Story 3.2, 3.7) ─────────────────────────
    const isEphemeral = request.lifecycleMode === 'ephemeral' || request.type === 'tool';
    const securityOptions = ContainerSecurityMapper.mapSecurityOptions(caps, isEphemeral);
