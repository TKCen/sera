//! Golden manifest tests — validate that sera-types can parse all YAML
//! files in contracts/manifests/ without error.

use sera_types::manifest::AgentTemplate;
use sera_types::policy::SandboxBoundary;
use std::fs;
use std::path::Path;

fn contracts_dir() -> &'static Path {
    // Tests run from the crate root (rust/crates/sera-types/),
