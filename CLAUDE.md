...
- **Chat routes are container-only (no in-process fallback)**: Since #428, `POST /api/chat` routes exclusively through the agent container's chat server. There is no fallback to `WorkerAgent.process()` or `BaseAgent.processStream()`. If the container is unavailable, the route returns 503. `SandboxManager.waitForChatReady()` polls the container's `/health` endpoint with exponential backoff before marking it ready. `WorkerAgent.process()` and `BaseAgent.processStream()` are still used by channel adapters, process flows, and `openai-compat.ts`.
...
- **pg-boss v9+ requires `createQueue()` before `schedule()`**: Each cron schedule uses its UUID as the pg-boss queue name. Calling `boss.schedule(id, cron, data)` without first calling `boss.createQueue(id)` throws "Queue not found". `ScheduleService.ensureQueueAndSchedule()` wraps both calls. The `agent-schedule` worker queue (created in `start()`) is separate from per-schedule queues.
- **Token budget `0` = unlimited**: `MeteringService.checkBudget()` treats a quota of `0` as unlimited (skips enforcement for that window). Manifest `spec.resources.maxLlmTokensPerHour/Day` values are synced to `token_quotas` on instance creation and template upsert. The PATCH `/api/budget/agents/:id/budget` endpoint accepts `0` or `null` to set unlimited. Defaults (100K hourly, 1M daily) are exported from `MeteringService.ts`.
