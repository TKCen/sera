...
- **Rebuilding the live container from a worktree branch**: The compose project name defaults to the directory basename, which is `rust` for the main checkout *and* every worktree — so `docker compose --project-name rust -f <worktree>/rust/docker-compose.sera.yml -f /home/entity/projects/sera/rust/docker-compose.override.yaml --env-file /home/entity/projects/sera/rust/.env up -d --build sera` replaces the same `rust-sera-1` container with an image built from the worktree's source. Watch the `./sera.yaml` mount: it resolves relative to the compose file, so the worktree's (gitignored) `sera.yaml` must carry the live operator config. The live compose service is `sera` (the `sera-gateway` crate is installed as the `sera` binary in the image).
- **OpenAI-compat backends require `parameters.properties`**: LM Studio returns HTTP 400 (`invalid_type` on `function.parameters.properties`) for any tool whose schema omits the `properties` key, failing the whole turn. `FunctionParameters.properties` therefore always serializes (`{}` when empty) — do not re-add `skip_serializing_if` to it (regression test: `empty_function_parameters_serialize_properties_key` in sera-types).
