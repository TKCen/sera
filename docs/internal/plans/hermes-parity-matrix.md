...
| Lane-busy short-circuit | `POST /api/chat` (concurrent) | Second concurrent turn for the same agent returns HTTP 429 with `Retry-After: 15` and `{error: "rate_limited", reason: "lane_busy", retry_after_secs: 15}`. The first turn still completes successfully once the lane clears. Operator-visible "blocked" state. |
| Session isolation per agent | `POST /api/chat` (two-agent manifest) | Each agent receives a distinct `session_id`; the SQLite `transcript` table holds exactly one user row per session, and each row carries its own agent's message only (no cross-agent bleed). |
...
Owning crate: `rust/crates/sera-e2e-harness`. Owning tests:
`tests/hermes_parity_baseline.rs` (readiness + authenticated chat + sanitizer
+ freshness), `tests/hermes_parity_response_sanitizer.rs` (sanitizer
regression with a deliberately reasoning-style mock reply), and
`tests/hermes_parity_api_smoke.rs` (lane-busy 429 negative + per-agent
session isolation). All gated behind `--features integration`.
Owning report template: `docs/internal/sessions/hermes-parity-baseline-template.md`
(template for operator-run live captures; the tests themselves are the
canonical CI/regression gate). The `artifacts/` working directory is
gitignored, so operator-run capture instances are kept out-of-tree by
convention.
...
|---|---|---|---|---|---|---|---|
| 1 | API chat intake (`/api/chat`) | `sera-gateway` chat handler (`crates/sera-gateway/src/bin/sera.rs::chat_handler`); session-scoped through gateway-owned SQLite. | Stable JSON contract (`session_id`, `response`, `usage`), SSE streaming events, `POST /api/chat/cancel`, response sanitizer applied before operator-visible bytes. **Differentiation:** gateway-owned session and audit, no runtime-side completion text reaching operator without sanitization. | `hermes_parity_baseline.rs::hermes_parity_baseline_gate` (positive: `/api/health/ready` + authenticated `/api/chat` + sanitizer + freshness nonce) **plus** `hermes_parity_api_smoke.rs` (negative + isolation: `hermes_parity_api_lane_busy_returns_429` proves concurrent lane-busy turns return HTTP 429 with `Retry-After: 15` and `{error: "rate_limited", reason: "lane_busy", retry_after_secs: 15}` envelope; `hermes_parity_api_session_isolation_per_agent` proves a two-agent manifest hands out distinct `session_id`s and isolates user-transcript rows per session). Auth-enforced harness profile is a follow-up (see recommended new beads below). | `sera-duo3` (CI smoke pipeline), reinforced by `sera-yj18` (streaming tool-call edge cases). | BASELINE | P2 |
...
| 8 | Memory / session search | Tier-1 MemoryBlock + Tier-2 semantic recall scaffolding in `sera-runtime/src/context_engine`; gateway-owned durable memory in `sera-db`. Transcript recovery: `GET /api/sessions/:id/transcript` now distinguishes "session_not_found" (HTTP 404 with `{error: "session_not_found", session_id, message}`) from "session exists but empty" (HTTP 200 `[]`) so a resume target that doesn't exist fails visibly instead of silently. | Two-tier injection: compact MemoryBlock + on-demand semantic search tool; session transcript search; cross-session operator state with provenance. **Differentiation:** scoped retrieval + audit; memory writes governed (`sera-rj4z`); transcript recovery failures are visible, not silently empty. | Cross-session retrieval test under `sera-e2e-harness` once `sera-rj4z` lands; visible-failure-on-unknown-session covered today by `sera-gateway` unit test `transcript_endpoint_404s_unknown_session_with_envelope`. | `sera-rj4z` (vector semantic memory + cross-session continuity); transcript-recovery visible-failure shipped alongside this matrix update. | IN_PROGRESS | P2 |
...
| 10 | Delegation / subagents | Subagent spawn/list/status scaffolding in `sera-runtime`; OMC / Claude / Codex / Hermes profiles can be launched. Pattern A vs Pattern B distinction not yet enforced. **GoalRun envelope mapped:** the PR closeout watcher is documented as a SERA-native GoalRun (`docs/internal/plans/hermes-parity-goalrun-byoh.md`); ready-queue gate + delegation-evidence path proven by `sera-workflow::tests::pr_closeout_watcher_goalrun_emits_evidence_on_terminal`. | BYOH harnesses available with Pattern A audit when possible and Pattern B explicitly labeled opaque; subagent spawn becomes a GoalRun step rather than a loose helper call. **Differentiation:** GoalRun accountability, not chat-style spawn. | Spawn → status → close-out smoke under `sera-e2e-harness` with audit assertion; tracked under a new bead. The GoalRun envelope itself is proven in-crate by `pr_closeout_watcher_goalrun_emits_evidence_on_terminal`. | New bead recommended (working name: `sera-delegation-goalrun`); first parity slice anchored at `docs/internal/plans/hermes-parity-goalrun-byoh.md`. | IN_PROGRESS | P2 |
| 11 | Workflows / cron / webhooks | Workflow engine scaffolding in `sera-workflow`; cron/event/threshold/manual trigger work tracked in existing workflow beads. Webhook intake exists; sanitization layer (`sera-ctag`) not yet wired. **GoalRun envelope mapped:** see `docs/internal/plans/hermes-parity-goalrun-byoh.md`; the PR closeout watcher is now expressed as a `WorkflowTask` + `AwaitType::GhPr` + `SubagentDelegationObserver` triad. | Cron / event / threshold / manual trigger + watchdog / no-agent script equivalent; webhook sanitization layer in front of prompt/context construction. **Differentiation:** workflow tasks are GoalRuns with stop conditions, evidence, and closeout. | Workflow trigger smoke + webhook sanitizer smoke under `sera-e2e-harness`. The GoalRun envelope itself is proven in-crate by `pr_closeout_watcher_goalrun_emits_evidence_on_terminal`. | `sera-ctag` (webhook sanitization), plus a new workflow-parity bead recommended for the GoalRun acceptance test; first parity slice anchored at `docs/internal/plans/hermes-parity-goalrun-byoh.md`. | IN_PROGRESS | P2 |
