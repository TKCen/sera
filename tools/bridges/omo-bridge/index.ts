/**
 * OMO Bridge — invokes the `opencode` CLI for each task received from sera.
 *
 * Extends BridgeBase. The execute() method:
 *   1. Writes the task prompt to a temp file in the worktree
 *   2. Spawns `opencode -p <prompt>`
...
class OmoBridge extends BridgeBase {
  constructor() {
    super({
      toolName: 'omo',
      displayName: 'OMO Bridge (host)',
    });
  }
