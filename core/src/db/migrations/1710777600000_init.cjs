...
exports.up = (pgm) => {
  ...
  pgm.sql(
    'CREATE INDEX IF NOT EXISTS chat_sessions_agent_updated_idx ON chat_sessions (agent_name, updated_at DESC)'
  );
  ...
  pgm.sql(
    'CREATE INDEX IF NOT EXISTS token_usage_agent_created_idx ON token_usage (agent_id, created_at DESC)'
  );
  ...
  pgm.sql(
    'CREATE UNIQUE INDEX IF NOT EXISTS skill_packages_name_version_idx ON skill_packages (name, version)'
  );

  // ── Knowledge Merge Requests (Epic 08) ────────────────────────────────────
  pgm.createTable(
    'knowledge_merge_requests',
    {
      id: { type: 'uuid', primaryKey: true, default: pgm.func('gen_random_uuid()') },
  ...
  pgm.sql(
    'ALTER TABLE agent_instances ADD COLUMN IF NOT EXISTS circle_id uuid REFERENCES circles ON DELETE SET NULL'
  );
