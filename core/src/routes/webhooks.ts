 ...
 */
export function createWebhooksRouter(webhooksService: WebhooksService, authMiddleware?: any): Router {
  const router = Router();
    ...
    router.post('/', authMiddleware, async (req: Request, res: Response) => {
      try {
        ...
        if (!name || !urlPath || !secret || !eventType) {
          return res.status(400).json({ error: 'name, urlPath, secret, and eventType are required' });
        }
  ...
  router.post('/incoming/:slug', async (req: Request, res: Response) => {
    try {
      ...
      return res.status(401).json({ error: 'Missing or invalid X-Sera-Signature or X-Sera-Timestamp' });
    } catch (err: any) {
