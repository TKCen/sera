...
- JWT expiry should be generous (24h or the agent's expected max runtime) — heartbeat validates liveness, not the token
...
- [ ] sera-core background job checks for agents with `status: running` and `last_heartbeat_at` older than configurable threshold (default: 120s)
- [ ] Agents exceeding threshold marked `status: unresponsive`
...
- [ ] For `one-time` and `session` filesystem grants: sera-core acts as a **host-side file proxy**
  - `file-read`, `file-write`, `file-list` tool calls for paths covered by a session/one-time grant are routed to sera-core (not executed inside the container)
  - sera-core reads/writes the path on the host filesystem and returns the result
  - The container never needs a bind mount for these operations
...
- [ ] Background job runs every 15 minutes: checks actual disk usage of each active agent's workspace directory via `du -s`
