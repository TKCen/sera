...
describe('CapabilityResolver', () => {
  ...
  beforeEach(() => {
    registryMock = {
      ...
      getNamedList: vi.fn(),
      listAlwaysEnforcedNamedLists: vi.fn(),
    };
  ...
  it('resolves NamedList references recursively', async () => {
    ...
    const result = await resolver.resolve('id');
    expect(result.resolvedCapabilities.network.outbound).toEqual(['google.com', 'github.com']);
  });
  ...
  it('allows empty array or object in manifest even if undefined in boundary', async () => {
    ...
    const result = await resolver.resolve('id');
    expect(result.resolvedCapabilities.fs.write).toBe(false);
    expect(result.resolvedCapabilities.net).toBe(false);
  });
  ...
  it('deepMerge handles $append and $remove for skills', async () => {
    ...
    const result = await resolver.resolve('id');
    expect(result.spec.skills).toContain('base-skill');
    expect(result.spec.skills).toContain('new-skill');
    expect(result.spec.skills).not.toContain('old-skill');
  });
  ...
  it('deepMerge handles scalar $append/$remove', async () => {
    ...
    const result = await resolver.resolve('id');
    expect(result.spec.skills).toEqual(['scalar-skill']);
  });
});
