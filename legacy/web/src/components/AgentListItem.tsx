...
export const AgentListItem: React.FC<AgentListItemProps> = ({
  ...
  return (
    <div className="sera-card relative flex items-center gap-4 px-4 py-3 group" style={style}>
      ...
      {/* Quick actions */}
      <div className="relative z-10 flex items-center gap-1 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus-within:opacity-100 transition-opacity">
      ...
      <Link
        ...
        aria-label={`View details for agent ${agent.display_name ?? agent.name}`}
      />
    </div>
  );
};
