...
describe('PipelineService', () => {
  ...
  describe('create', () => {
    it('creates a new pipeline and returns it', async () => {
      ...
      vi.mocked(query)
        .mockResolvedValueOnce({ rows: [] } as unknown as import('pg').QueryResult) // first call (INSERT ignores return rows currently)
        .mockResolvedValueOnce({ rows: [mockCreatedRow] } as unknown as import('pg').QueryResult); // second call is get()
  ...
  describe('get', () => {
    it('returns null if pipeline is not found', async () => {
      vi.mocked(query).mockResolvedValue({ rows: [] } as unknown as import('pg').QueryResult);
