...
//! render a descriptions-only block that points the agent at the `skill_view`
//! and `skills_list` tools.
//!
//! The block is intentionally body-free: it advertises availability, not
//! contents. The agent pulls full instructions on demand through `skill_view`.
...
fn render_block(entries: &[(String, String)]) -> String {
    let header = "## Skills (mandatory)\n\
        The following skills are available in this runtime. Before answering, check whether one matches the task. If a skill matches, you MUST load its full instructions with the `skill_view` tool (pass its name); err on the side of loading. Use `skills_list` to re-check availability.\n\n";
    ...
    for (idx, (name, description)) in entries.iter().enumerate() {
        ...
        // after truncation.
        let pointer = format!("…and {remaining} more (use skills_list)\n");
...
mod tests {
    ...
    async fn caps_block_with_pointer_when_many_skills() {
        ...
        assert!(block.contains("more (use skills_list)"));
    }
}
