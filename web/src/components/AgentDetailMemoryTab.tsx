...
import { useAddMemoryEntry } from '@/hooks/useMemoryExplorer';
...
export function MemoryTab({ id }: { id: string }) {
  ...
  const addMemoryEntry = useAddMemoryEntry();
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      const tags = newEntry.tags
        ...
        .map((t) => t.trim())
        .filter(Boolean);
      await addMemoryEntry.mutateAsync({
        type: newEntry.type,
        data: {
  ...
  }, [newEntry, refetch, addMemoryEntry]);
